...
model_layers <- c(
  dem_preproc, dem, chm, slope_aspect$aspect, mrvbf$mrvbf, mrvbf$mrrtf, tri, tca, sca, so, cn, twi, overland_flow$distance, openness$pos, openness$neg)
...
sf_grouse <- st_read("Sharp-tailed grouse.gpkg") 
grouse <- vect(sf_grouse) 
mapview(grouse)


# Perform data extraction at each point in each raster layer
ldb_data <- terra::extract(model_layers, grouse, bind = TRUE) %>% 
  st_as_sf()
mapview(ldb_data)
