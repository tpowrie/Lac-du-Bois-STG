...
openness_p <- rast("ta/ta/openness_pos.tif")
openness_n <- rast("ta/ta/openness_neg.tif")
...
#best model
best_lrn_prob <- rr_prob$learners[[which.min(mod_scores_prob$classif.auc)]]$learner
best_lrn_prob
...
imp_plot_prob <- ggplot(imp, aes(x = Importance, y = Variable)) + 
  ...
  scale_y_discrete(labels = c("Canopy Height Model", "MRVBF", "Negative Topographic Openness", "Slope", "Aspect", "Terrain Ruggeness Index", "Topographic Wetness Index", "Positive Topographic Openness", "MRRTF", "Overland Flow Distance"))
...
plot(prediction_prob_terra_stg)
